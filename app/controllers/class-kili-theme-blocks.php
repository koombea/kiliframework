...
class Kili_Theme_Blocks {
  ...
  public function add_blocks_to_wp( $location ) {
    ...
    if( function_exists( 'acf_add_local_field_group') && $location ) {
      ...
      $layouts = array();
      if ( $blocks_file_path or $is_defined_alternative_page_blocks ) {
        ...
        $meta = array(
          ...
          'fields' => array (
            array (
              ...
              'instructions' => 'Place and edit your blocks. Add photos to your blocks, change text colors and fonts.',
              'required' => 0,
              'conditional_logic' => 0,
              'wrapper' => array (
                'width' => '',
          ...
          'location'         => array(),
          'active'        => 1,
          'description'       => 'Kili Framework page builder',
        );
  ...
  private function get_file_layout( $blocks_path, $file ) {
    ...
    if ($json_to_php != NULL) {
      ...
      else if ( isset($current_layout[0][0]) ) {
   ...
   * Get blocks layout
   *
   * @param [type] $blocks_path Block files path
