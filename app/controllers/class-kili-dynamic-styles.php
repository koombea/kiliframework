...
class Kili_Dynamic_Styles {
   ...
   *
   * @param [type] $field Block field
   * @param [type] $page_id Page id
   * @param [type] $layout Block layout
   * @param [type] $block_position Block position
  ...
  private function replace_placeholders( $field, $page_id, $layout, $block_position ) {
    ...
    if ( strcasecmp( $styles, '' ) != 0 ) {
      ...
      $replacements = array_merge( $replacements, get_array_replacement( $field ) );
   ...
   *
   * @param [type] $array Placeholders array
  ...
  private function get_array_replacement( $array, $prefix='' ) {
    ...
    foreach ( $array as $key => $value ) {
      ...
      else if ( is_string( $value ) && strcasecmp( trim( $value ),'' ) != 0 ) {
